...
@section('page_subtitle', 'Suivi des affectations et restitutions')
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200">
    <div class="px-4 lg:px-6 py-4 border-b border-slate-200">
        <div class="flex flex-col sm:flex-row gap-3">
            <form method="GET" action="{{ route('gestionnaire.affectations.index') }}"
                ...
                <select name="client_id" class="border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 flex-1 sm:flex-none">
                ...
                <input type="date" name="date_debut" value="{{ request('date_debut') }}"
                       class="border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 flex-1 sm:flex-none">
                ...
                <a href="{{ route('gestionnaire.affectations.index') }}" class="text-slate-400 hover:text-slate-600 text-sm">
        ...
        <table class="w-full text-sm" style="min-width:600px">
            <thead class="bg-slate-50 border-b border-slate-200">
                <tr>
                    ...
                    <th class="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase">Date & Heure</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase hidden md:table-cell">Restitution</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-4 py-3">
                        <p class="font-medium text-slate-800">{{ Str::limit($aff->materiel?->designation, 22) }}</p>
                        <p class="text-xs text-slate-400 font-mono">{{ $aff->materiel?->code_identification }}</p>
                    </td>
                    <td class="px-4 py-3">
                        <p class="font-medium text-slate-700">{{ Str::limit($aff->client?->nom, 16) }}</p>
                    </td>
                    <td class="px-4 py-3 text-center font-bold text-slate-700">{{ $aff->quantite }}</td>
                    <td class="px-4 py-3">
                        <p class="text-slate-700 text-xs">{{ $aff->date_affectation?->format('d/m/Y') }}</p>
                        <p class="text-slate-400 text-xs">{{ $aff->heure_affectation }}</p>
                    </td>
                    <td class="px-4 py-3 text-slate-500 text-xs hidden md:table-cell">
                        {{ $aff->date_restitution?->format('d/m/Y') ?? '—' }}
                    </td>
                    <td class="px-4 py-3">
                        <span class="px-2 py-1 rounded-full text-xs font-medium {{ $aff->statut_badge }}">
                            {{ $aff->statut_label }}
                        </span>
                    </td>
                    <td class="px-4 py-3 text-center">
                        ...
                        <form method="POST" action="{{ route('gestionnaire.affectations.restituer', $aff->id) }}"
                            ...
                            <button type="submit"
                                ...
                                <span class="hidden sm:inline ml-1">Restituer</span>
                            </button>
                        </form>
                        @else
                ...
                <tr>
                    <td colspan="7" class="px-4 py-12 text-center text-slate-400">
                        ...
                        <p class="font-medium">Aucune affectation</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
    ...
    <div class="px-4 lg:px-6 py-4 border-t border-slate-100">{{ $affectations->links() }}</div>
</div>
