...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-5">
    ...
    <div class="xl:col-span-1 space-y-4">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-xl p-4">
            <h4 class="text-sm font-semibold text-blue-800 mb-2">
                <i class="fa-solid fa-link mr-1"></i> Lien portail client
            </h4>
            <p class="text-xs text-blue-600 mb-3">
                Partagez ce lien au client par SMS ou email. Il pourra consulter ses matériels et signaler des problèmes.
            </p>
            <div class="bg-white rounded-lg border border-blue-200 p-2 mb-2">
                <p class="text-xs font-mono text-slate-600 break-all" id="lienPortail">{{ $client->lien_portail }}</p>
            </div>
            <div class="flex gap-2">
                <button onclick="copierLien()"
                        class="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-xs py-2 rounded-lg font-medium transition-colors flex items-center justify-center gap-1">
                    <i class="fa-solid fa-copy"></i> Copier le lien
                </button>
                <a href="{{ $client->lien_portail }}" target="_blank"
                   class="flex-1 border border-blue-300 text-blue-700 text-xs py-2 rounded-lg font-medium transition-colors flex items-center justify-center gap-1 hover:bg-blue-50">
    ...
    <div class="xl:col-span-2 space-y-5">
        ...
        {{-- Signalements du client --}}
        @if(isset($client->signalements) && $client->signalements->isNotEmpty())
        <div class="bg-red-50 border border-red-200 rounded-xl p-4">
            <h3 class="text-sm font-semibold text-red-800 mb-3">
                <i class="fa-solid fa-triangle-exclamation mr-2"></i>
                Signalements de ce client ({{ $client->signalements->count() }})
            </h3>
            <div class="space-y-2">
                @foreach($client->signalements->take(3) as $sig)
                <div class="bg-white rounded-lg p-3 flex items-start gap-3">
                    <span class="text-lg">{{ explode(' ', $sig->type_label)[0] }}</span>
                    <div class="flex-1">
                        <p class="text-xs font-semibold text-slate-700">{{ $sig->materiel?->designation }}</p>
                        <p class="text-xs text-slate-500 mt-0.5">{{ Str::limit($sig->description, 80) }}</p>
                        <p class="text-xs text-slate-400 mt-1">{{ $sig->created_at->diffForHumans() }}</p>
                    </div>
                    <span class="text-xs px-2 py-0.5 rounded-full {{ $sig->statut_badge }}">{{ $sig->statut_label }}</span>
                </div>
                @endforeach
            </div>
            <a href="{{ route('gestionnaire.signalements.index') }}" class="text-xs text-red-600 hover:underline mt-2 inline-block">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            ...
            @if($client->affectationsEnCours->isEmpty())
                <div class="text-center py-6 text-slate-400">
                    ...
                    <p class="text-sm">Aucun matériel assigné actuellement</p>
                </div>
            @else
                        ...
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="py-2 text-center">
                                <form method="POST" action="{{ route('gestionnaire.affectations.restituer', $aff->id) }}"
                                    ...
                                    @csrf
                                    <button type="submit"
                                            class="bg-green-100 hover:bg-green-200 text-green-700 px-2.5 py-1 rounded font-medium transition-colors">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <h3 class="text-sm font-semibold text-slate-700 mb-4">
                <i class="fa-solid fa-clock-rotate-left text-slate-400 mr-2"></i>Historique complet
            </h3>
                ...
                <table class="w-full text-xs" style="min-width:400px">
                    <thead class="sticky top-0 bg-white">
                        <tr class="border-b border-slate-100">
                            ...
                            <th class="text-center py-2 text-slate-500 font-medium">Qté</th>
                            <th class="text-left py-2 text-slate-500 font-medium">Assigné</th>
                            <th class="text-left py-2 text-slate-500 font-medium">Restitué</th>
...
<script>
function copierLien() {
    const lien = document.getElementById('lienPortail').textContent.trim();
    navigator.clipboard.writeText(lien).then(() => {
        alert('Lien copié ! Vous pouvez le partager au client.');
    });
}
</script>
@endpush
