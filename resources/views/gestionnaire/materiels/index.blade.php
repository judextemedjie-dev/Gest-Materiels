...
@section('page_title', 'Matériels')
@section('page_subtitle', 'Inventaire du parc matériel')
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200">
    ...
    <div class="px-4 lg:px-6 py-4 border-b border-slate-200">
        <div class="flex flex-col sm:flex-row gap-3">
            <form method="GET" action="{{ route('gestionnaire.materiels.index') }}"
                  class="flex flex-wrap gap-2 items-center flex-1">
                <div class="relative w-full sm:w-auto">
                    <i class="fa-solid fa-search absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs"></i>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Rechercher..."
                           class="pl-8 pr-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 w-full sm:w-44">
                </div>
                <select name="categorie_id" class="border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 flex-1 sm:flex-none">
                ...
                <a href="{{ route('gestionnaire.materiels.index') }}" class="text-slate-400 hover:text-slate-600 text-sm">
                    <i class="fa-solid fa-xmark"></i>
                </a>
                @endif
            </form>
            <a href="{{ route('gestionnaire.materiels.create') }}"
               class="bg-teal-600 hover:bg-teal-700 text-white text-sm px-4 py-2 rounded-lg flex items-center gap-2 transition-colors whitespace-nowrap self-start sm:self-auto">
                <i class="fa-solid fa-plus"></i> <span class="hidden sm:inline">Ajouter</span><span class="sm:hidden">Nouveau</span>
            </a>
        </div>
    </div>
        ...
        <table class="w-full text-sm" style="min-width:600px">
            <thead class="bg-slate-50 border-b border-slate-200">
                <tr>
                    ...
                    <th class="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase hidden md:table-cell">Catégorie</th>
                    ...
                    <th class="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase hidden lg:table-cell">Localisation</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-4 py-3 font-medium text-slate-800">{{ Str::limit($m->designation, 28) }}</td>
                    <td class="px-4 py-3 font-mono text-xs text-slate-500">{{ $m->code_identification }}</td>
                    <td class="px-4 py-3 text-slate-600 hidden md:table-cell">{{ $m->categorie?->nom ?? '—' }}</td>
                    <td class="px-4 py-3 text-center">
                        <span class="text-base font-bold {{ $m->quantite_stock <= 2 ? 'text-red-600' : 'text-slate-700' }}">
                            {{ $m->quantite_stock }}
                        </span>
                        @if($m->quantite_stock > 0 && $m->quantite_stock <= 2)
                            <i class="fa-solid fa-triangle-exclamation text-amber-500 text-xs ml-0.5"></i>
                        @elseif($m->quantite_stock == 0)
                            <i class="fa-solid fa-circle-xmark text-red-500 text-xs ml-0.5"></i>
                        @endif
                    </td>
                    <td class="px-4 py-3">
                        <span class="px-2 py-1 rounded-full text-xs font-medium {{ $m->statut_badge }}">
                            {{ $m->statut_label }}
                        </span>
                    </td>
                    <td class="px-4 py-3 text-slate-500 text-xs hidden lg:table-cell">{{ $m->localisation ?? '—' }}</td>
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('gestionnaire.materiels.show', $m) }}"
                               class="text-blue-500 hover:text-blue-700" title="Détails">
                                <i class="fa-solid fa-eye"></i>
                            </a>
                            <a href="{{ route('gestionnaire.materiels.edit', $m) }}"
                               class="text-amber-500 hover:text-amber-700" title="Modifier">
                                <i class="fa-solid fa-pen-to-square"></i>
                            </a>
                            <form method="POST" action="{{ route('gestionnaire.materiels.destroy', $m) }}"
                                  onsubmit="return confirm('Archiver ce matériel ?')">
                                @csrf @method('DELETE')
                                <button type="submit" class="text-slate-400 hover:text-red-500" title="Archiver">
    ...
    <div class="px-4 lg:px-6 py-4 border-t border-slate-100">{{ $materiels->links() }}</div>
</div>
